...
class Simulator:
    ...
    def step(self):
        ...
        if event.type == market_event_type.SUBMIT:
            fills, remaining = self.order_book.add_order(event.order)
        ...
        elif event.type == market_event_type.CANCEL:
            self.order_book.cancel_order(event.order_id)
            return [], None  # Return empty fills and no remaining order for CANCEL
        elif event.type == market_event_type.REDUCE:
            self.order_book.reduce_order(event.order_id, event.quantity)
            return [], None  # Return empty fills and no remaining order for REDUCE
